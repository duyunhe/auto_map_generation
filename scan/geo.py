...
def hausdorff_dist(a_list, b_list):
    """
    a --> b
    :param a_list: list of [x, y]
    :param b_list: 
    :return: 
    """
    max_dist = 0
    for pa in a_list:
        ...
        for pb in b_list:
            if last_pt is not None:
                dist = point2segment_xy(pa, last_pt, pb)
...
def hausdorff_par_dist(a_list, b_list):
    """
    a --> b
    :param a_list: list of [x, y]
    :param b_list: 
    ...
    for pa in a_list:
        ...
        for pb in b_list:
            if last_pt is not None:
                dist = point2segment_par_xy(pa, last_pt, pb)
                cur_dist = min(cur_dist, dist)
            last_pt = pb
        # print pa, cur_dist
        max_dist = max(max_dist, cur_dist)
...
def point2segment_par_xy(point, sp0, sp1):
    x, y = point[:]
    x0, y0 = sp0[:]
    x1, y1 = sp1[:]
    ap, ab = np.array([x - x0, y - y0]), np.array([x1 - x0, y1 - y0])
...
def point2segment_xy(point, segment_point0, segment_point1):
    """
    :param point: Point
    :param segment_point0: 
    :param segment_point1: 
...
def get_cross_point_v0(p0, p1, p2, p3):
    """
    获得两线段交点（在延长线上的交点）
    :return: d 左手边>0 右手边<0 平行=0    px, py
    """
    a0, b0, c0 = get_line_equation(p0, p1)
    a1, b1, c1 = get_line_equation(p2, p3)
...
def inter_seg(p0, p1, p2, p3):
    ...
    if (max(p0.x, p1.x) >= min(p2.x, p3.x) and max(p2.x, p3.x) >= min(p0.x, p1.x)
       and max(p0.y, p1.y) >= min(p2.y, p3.y) and max(p2.y, p3.y) >= min(p0.y, p1.y)):
        # 跨立实验
            if (cross(p0, p1, p2) * cross(p0, p1, p3) < 0
...
def main_test():
    a_list = [[0.0, 0.0], [1.0, 0.0]]
    b_list = [[3.0, 1.0], [6.0, 1.0]]
    print hausdorff_par_dist(a_list, b_list)
